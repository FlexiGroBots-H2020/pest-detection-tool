...
if __name__ == "__main__":
    ...
    if args.det_model == "Detic":
        ...
        model_predictor = load_model_grdSAM()
    else:
        model_predictor = YOLO("yolov8l.pt")  # load an official model
        model_predictor = YOLO("models/best_yolov8L_pest.pt")  # load a custom model
    ...
    for img_path in list_images_paths:
        ...
        if args.full_pipeline:
            ...
            try:
                ...
                img_out_bbox, img_out_mask, mask, cont_det, pred = predict_img(img_zoi_crop, args, model_predictor, save=True, save_path=output_folder, img_o=img_ori_np, fruit_zone=fruit_bbox, cont_det=cont_det)
                ...
                try:
                    if args.det_model == "Detic":
                        ...
                        for ii in range(len(args.classes_grdSAM)):
                            clss_names.append(args.classes_grdSAM[ii])
                    else:
                ...
                if args.det_model == "Detic":
                    ...
                    if len(mask.shape)>2:
                        mask_final = (mask[:, :, 3] > 128).astype(int)
                    else: 
                        mask_final=mask
                else:
                    mask_final = mask
                ...
                grids = None
                logger.info("Error detecting: {}".format(e))
        ...
        try:
            ...
            with open(json_path, 'w') as f:
                ...
                grids = None
                logger.info("Error saving annotations: {}".format(e))
